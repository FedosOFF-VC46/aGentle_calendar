    ...
    <div>
      ...
      <div className="card">
        ...
        <div className="row" style={{ marginTop: 10 }}>
          ...
          <button className="btn" onClick={addMedication}>Добавить</button>
        </div>
      </div>
      ...
      {drafts.map((medication) => (
        <div key={medication.id} className="card">
          <div className="space">
            ...
            <button className="btn ghost" onClick={() => setDrafts((prev) => prev.filter((item) => item.id !== medication.id))}>Удалить</button>
          </div>
      ...
      <button className="btn success" onClick={() => onSave(drafts)}>Сохранить и пересчитать схему</button>
    </div>
  );
};